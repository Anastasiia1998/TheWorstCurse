...
- **`Hero.dc.html`** — card fields embedded in the page via Stripe Elements. Customer never leaves your site.
- **`Hero Checkout.dc.html`** — hosted Stripe Checkout: the Pay button sends them to Stripe's own payment page and back to your "Curse successfully sent." screen. Least PCI surface; also gets Apple Pay / Google Pay for free.

Pick one as your homepage in `vercel.json` (`rewrites` → `/`). Both use the same keys and the same server price table.

## What's in the project

- `api/create-checkout-session.js` — `POST`, creates a hosted Checkout session (used by the Checkout version).
- `api/checkout-status.js` — `GET`, asks Stripe whether a returning session was really paid before the success screen shows.
- `api/config.js` — `GET /api/config`, hands the browser your **publishable** key from the environment.
- `api/create-payment-intent.js` — `POST`, creates the PaymentIntent (used by the Elements version).
...
- `vercel.json` — serves the chosen page at `/`, no-cache on `/api/*`.
...
On success the form swaps to your existing **"Curse successfully sent."** screen. In the Checkout version you are redirected to Stripe, then returned to `?curse=paid&session_id=…`, which is verified server-side before that screen appears. The payment appears in Stripe → Payments with `curse_id`, `curse_text`, and `victim` in the metadata.
